...
| Fine-tuning versus five-shot B1-P1 | Incomplete | `results/b1_b2_qalb_dev_gpu_validation.md`; `results/b1_p1_final_attempt_failure_audit.md` | The 25-record development gate passed technically and the timeout-safe guard is merged. Issue #109's first final attempt failed before model or data access because its wrapper assumed an unavailable external `nvidia-smi` command; CUDA availability was not measured. | Repair and review the GPU preflight, then obtain a fresh exact-commit GO for one frozen 511-record final run. |
