...
| [Enhancing Text Editing for GEC (2025)](https://aclanthology.org/2025.acl-long.875/) | Derives language-independent edit tags from data and improves Arabic GEC quality and decoding efficiency | Four Arabic GEC benchmarks | Seq2edit models with data-derived edit operations | Official benchmark metrics and decoding speed | Advances the editing formulation; does not isolate natural, synthetic, and mixed supervision under one fixed model and equal record count |
| [Contextual Data Augmentation (2024)](https://aclanthology.org/2024.findings-acl.647/) | Addresses synthetic GEC error-distribution mismatch and noisy labels with contextual generation and relabeling | English GEC training and benchmarks | Contextual augmentation plus synthetic-label cleaning | Standard English GEC metrics | Establishes that synthetic-data construction and cleaning matter, but not for Arabic or a fixed equal-record source comparison |
...
| Tibyan | Large synthetic Arabic GEC corpus with reviewed correction-side text and aggregate ARETA analysis | 6,192 released aligned pairs; no official split, stable IDs, or released per-record labels | ChatGPT-assisted generation plus two-phase human review; ARETA applied afterward | Corpus-level ARETA frequencies | The release does not provide per-record category labels; correction-side review is not expert validation of every injected error or tag; reported category frequencies are highly skewed |
| [ARETA](https://aclanthology.org/2021.conll-1.47/) | Automatic Arabic error-type annotation with a published two-level inventory | Arabic Learner Corpus evaluation; parallel source/correction input | Rule, alignment, and morphological-analysis pipeline; 7 classes and 26 supported tags | Best reported 85.8 micro-F1 and 55.0 macro-F1 on held-out ALC | Automatic tags are not expert gold; rare tags, many-to-many alignment, split/merge evaluation, overlapping categories, and long-distance phenomena remain limitations |
