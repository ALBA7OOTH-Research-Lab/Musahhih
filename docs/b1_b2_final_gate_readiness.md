...
Status reviewed: 2026-07-26. Preparation only; no execution authorized.

## Decision

The scientific protocol and private inputs are ready for implementation review,
but the current final-evaluation runner should not be submitted to Kaggle yet.
It predates the timeout-safe persistence and continuation controls added after
the F2/F3 runtime failure.

The required next step is a narrow engineering issue that hardens B1-P1/B2-P1
final execution without changing either frozen prompt, demonstration bundle,
parser, decoding setting, input, or statistic.
...
## Missing engineering safety

`scripts/run_prompt_baseline.py` currently has no fixed Kaggle wall-time budget,
per-row `fsync`, atomic progress manifest, metric-free timed handoff, or
hash-verified resume path. The F2/F3 experience shows that a full 511-record GPU
run can exceed Kaggle's hard cutoff. B1-P1 also has a longer five-shot prompt,
so relying on a single uninterrupted process would be unsafe.

Before any final submission, implement and test:

1. capture of the wrapper start time before setup or network work;
2. a conservative safe-stop threshold below Kaggle's hard cutoff;
...
8. a fresh exact-commit owner GO for every submitted segment;
9. first-terminal-state preservation with no automatic retry; and
10. private outputs under ignored paths only.

Synthetic-fixture tests must cover interruption, malformed prefixes, hash
mismatch, reordered records, score mismatch, unexpected existing directories,
and successful multi-segment completion without reading Nahw-Passage.
...
1. a dedicated implementation issue and branch;
2. merged timeout-safe code and synthetic/static validation;
3. an independent review of the exact merge commit;
4. an owner GO naming that commit and the first authorized segment; and
5. a separate terminal audit before any continuation.
