#!/usr/bin/env python3
"""Build one private, timeout-safe B1-P1 Kaggle kernel without executing it."""
...
KERNEL_ID_PATTERN = re.compile(r"[a-z0-9][a-z0-9-]*/[a-z0-9][a-z0-9-]*")
DATASET_SOURCE_PATTERN = re.compile(r"[a-z0-9][a-z0-9-]*/[a-z0-9][a-z0-9-]*")
RUN_ID = "B1-P1__gemma3-4b-it__nahw-passage__s3407__r01"
WRAPPER_FILENAME = "b1_final_evaluation.py"
...
def build_wrapper(*, approved_commit: str, approval_reference: str) -> str:
    """Render the corpus-text-free wrapper for one independently approved segment."""
...
EXPECTED_BUNDLE_SHA256 = "{FINAL_B1_BUNDLE_SHA256}"
...
# Private inputs are discovered only after the restored runtime passes.
input_path = None
bundle_path = None
for candidate in Path("/kaggle/input").rglob("*"):
    ...
    if digest == EXPECTED_INPUT_SHA256:
        if input_path is not None:
            raise RuntimeError("duplicate frozen final input")
        input_path = candidate
    elif digest == EXPECTED_BUNDLE_SHA256:
        if bundle_path is not None:
            raise RuntimeError("duplicate frozen B1 bundle")
        bundle_path = candidate
if input_path is None or bundle_path is None:
    raise RuntimeError("exact frozen B1 private inputs were not attached")
print(
    {{
...
if status not in {{"complete", "incomplete_time_budget"}}:
    raise RuntimeError("B1-P1 evaluator did not produce an accepted terminal state")
...
def build_metadata(*, kernel_id: str, dataset_source: str) -> dict:
    """Return private P100 kernel metadata with one reviewed data source."""

    if not KERNEL_ID_PATTERN.fullmatch(kernel_id):
        raise KernelPreparationError("kernel id must be owner/slug")
    if not DATASET_SOURCE_PATTERN.fullmatch(dataset_source):
        raise KernelPreparationError("dataset source must be owner/slug")
