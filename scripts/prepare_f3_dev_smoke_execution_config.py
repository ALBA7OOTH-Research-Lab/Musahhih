#!/usr/bin/env python3
"""Create the private activation file for the F3-P1 development smoke."""
...
CONFIG_FILENAME = "f3_dev_smoke_execution_config.json"
CONFIRMATION = "RUN_F3_P1_PRIVATE_DEV_SMOKE_25"
COMMIT_PATTERN = re.compile(r"[0-9a-f]{40}")
APPROVAL_PATTERN = re.compile(
    r"https://github\.com/ALBA7OOTH-Research-Lab/Musahhih/"
    r"issues/93#issuecomment-[1-9][0-9]*"
)


class F3DevSmokeConfigError(ValueError):
...
def build_execution_config(
    *,
    ...
    if not COMMIT_PATTERN.fullmatch(approved_workflow_commit):
        raise F3DevSmokeConfigError(
            "Approved workflow commit must be 40 lowercase hex characters"
        )
    if not APPROVAL_PATTERN.fullmatch(approval_reference):
        raise F3DevSmokeConfigError(
            "Approval reference must be an issue #93 comment URL"
        )
    if confirmation != CONFIRMATION:
        raise F3DevSmokeConfigError("Private development confirmation mismatch")
    return {
...
def write_execution_config(config: dict[str, str], output_path: Path) -> Path:
    ...
    if output_path.name != CONFIG_FILENAME:
        raise F3DevSmokeConfigError(f"Output filename must be {CONFIG_FILENAME}")
