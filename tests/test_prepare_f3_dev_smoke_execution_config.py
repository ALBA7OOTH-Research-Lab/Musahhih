...
from scripts.prepare_f3_dev_smoke_execution_config import (
    CONFIG_FILENAME,
    CONFIRMATION,
    F3DevSmokeConfigError,
    build_execution_config,
    write_execution_config,
)


class PrepareF3DevSmokeExecutionConfigTests(unittest.TestCase):
    COMMIT = "a" * 40
    APPROVAL = (
        "https://github.com/ALBA7OOTH-Research-Lab/Musahhih/"
        "issues/93#issuecomment-123456"
    )

    def test_builds_only_frozen_f3_development_smoke(self):
    ...
    def test_rejects_invalid_commit_reference_and_confirmation(self):
        ...
        for update in (
            ...
            with self.subTest(update=update):
                with self.assertRaises(F3DevSmokeConfigError):
