...
zero-shot, five-shot, and expert-style prompt baselines in staged
...
also report earlier natural-only and untouched-model results and later
prompt-only baselines as explicitly staged comparisons rather than presenting
all six systems as one simultaneous experiment.
...
All baselines use the untouched 4-bit Gemma 3 4B instruction model at immutable
revision \texttt{316726ca\ldots{}d1fe}.  \textbf{B0} gives a concise Arabic
instruction to correct the highlighted token and return only that token.
\textbf{B1} prepends five QALB-training demonstrations selected by a
...
training seed and excludes retraining variance.  Comparisons involving B0,
B1, B2, or F1 were executed or consolidated at different stages and are
secondary.  Their intervals and tests are descriptive and are not treated as
a simultaneous preregistered six-arm family.
...
Prompt engineering helps the untouched model modestly: B2 exceeds B0 by 4.31
points (interval 1.96--6.85), while B1 and B0 are not distinguishable under the
paired analysis.  F1 and F3 exceed B2 in staged comparisons.  F2 and B2 differ
by only $-0.59$ points, with an interval spanning zero.  All arms produce 511
aligned parsed predictions with no empty output or parsing failure.  Suspicious
or multi-token outputs number 2 for F1, 20 for F2, 2 for F3, and 6 for B2;
they are retained unchanged when scoring.  We do not introduce a post-hoc
first-token parser.  Even crediting all 20 flagged F2 outputs would raise F2
only to 125/511 (24.46\%), leaving F3 ahead by 7.24 points; format warnings
cannot explain the full primary difference.

\section{Safety Diagnostics}

Before the F1 diagnostic was run, we froze two matched checks comparing B0
with F1.  First, on 154 corrected QALB-2015 L2 development sentences, each
record designates one Arabic-script token already present in the corrected
target.  F1 leaves the designated token unchanged in 78/154 cases (50.65\%),
compared with 43/154 (27.92\%) for B0.  The paired difference is 22.73 points
(95\% interval 12.34--33.12).  This indicates less overcorrection on this
selected development diagnostic, not on arbitrary correct Arabic.
...
different stages.  Hardware also differed for some historical comparisons:
the accepted B0 run used a T4, whereas fine-tuned and later prompt runs used
P100 GPUs.  Greedy decoding reduces but does not eliminate possible
hardware-specific variation.  Only F3--F2 is a simultaneously executed,
