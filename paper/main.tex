...
QALB, a deterministic highlighted-token view of synthetic Tibyan, and a
50:50 mixture.  We freeze data
...
31.70\% exact accuracy versus 20.55\% for synthetic-only.  The preregistered
paired difference is 11.15 percentage points (95\% paired-bootstrap interval:
7.05--15.26; exact McNemar $p=2.15\times10^{-7}$).  A post-hoc five-seed
cohort preserves the direction in every seed (mean difference 10.29 points,
SD 1.45).  Natural-only reaches 28.38\%; the observed 3.33-point mixed
advantage is not established by the frozen interval.  Natural-only and mixed
also outperform prompt baselines in staged comparisons.  These results show
that supervision composition materially affects this task, while strict
single-reference scoring, one model, and absent expert error analysis limit
generality.
\end{abstract}
...
\begin{itemize}
    ...
    \item a post-hoc five-seed robustness cohort in which the mixed arm
    exceeds the synthetic-only arm in every seed; and
...
paired F2/F3 replicas use seeds 3407--3411, the frozen training recipe, and the
same checkpoint rule.  Training uses A100 GPUs, and all ten selected adapters
are evaluated from record zero on uniform RTX 3090 GPUs.  This cohort was run
after the primary result and is therefore robustness evidence rather than a
...
percentages; SD is the sample standard deviation across seeds. These adapters
were trained on A100s and evaluated uniformly on RTX 3090s.}
\label{tab:multiseed}
\end{table}
...
direction and magnitude without replacing the primary analysis.
...
error categories.  The primary interval remains conditional on seed 3407, but
the post-hoc five-seed cohort partly addresses training randomness: every
paired difference is positive.  Because the primary and robustness cohorts
use different hardware paths, this extension does not decompose every runtime
or hardware contribution.  Although F2 contains more tokens than F3, totals
do not control length, domain, or source quality.  We do not run QALB test, so
our accuracy is not comparable to full-sentence M2 scores.
...
in every seed.  Natural-only supervision also exceeds
