...
recipe, and the same checkpoint rule.  Training uses A100 GPUs, and all ten
selected adapters are evaluated from record zero on uniform RTX 3090 GPUs.
The seed-3407 replica does not reuse either original P100 adapter.  This cohort
...
percentages; SD is the sample standard deviation across seeds. All rows are
independent A100 retrains evaluated uniformly on RTX 3090s; ``3407-A100'' is
not the original P100 primary run.}
\label{tab:multiseed}
\end{table}
...
in every seed.  Natural-only supervision also exceeds synthetic-only
...
natural development distribution could still favor F3 over F2.  We do not yet
report a fixed-epoch sensitivity, so the multi-seed cohort addresses training
randomness but not this checkpoint-policy concern.
...
system reported in Tables~\ref{tab:main-results}--\ref{tab:paired} and
Figure~\ref{fig:accuracy} was evaluated on P100 hardware.  The separate
post-hoc cohort in Table~\ref{tab:multiseed} was trained on A100s and evaluated
uniformly on RTX 3090s.
